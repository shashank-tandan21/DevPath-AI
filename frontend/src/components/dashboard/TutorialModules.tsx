...
import { Link2, Loader2, Sparkles, Youtube, MessageSquare, Send, Maximize2, Minimize2, X } from "lucide-react";
...
const TutorialModules = ({
  ...
  onAnalysisComplete?: (newId?: string) => void,
  ...
  // Load existing module if initialModuleId is provided
  useEffect(() => {
    if (initialModuleId) {
      const fetchModule = async () => {
        try {
          setIsAnalyzing(true);
          const response = await fetch(`/api/tutorial-modules/${initialModuleId}`);
          if (!response.ok) throw new Error("Failed to fetch module");
          const module = await response.json();

          if (module) {
            setUrl(module.source_url || `https://www.youtube.com/watch?v=${module.source_video_id}`);
            setVideoId(module.source_video_id);
            setSummary(module.content?.raw_markdown || "");
            setChatHistory([
              { role: 'ai', content: "Hi! I've loaded this existing module for you. What specific questions do you have about the content?" }
            ]);
          }
        } catch (err) {
      ...
      // Reset if no module ID (new analysis)
      setUrl("");
      setVideoId(null);
      setSummary(null);
      setChatHistory([]);
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (onAnalysisComplete) onAnalysisComplete(data.moduleData?.id);
    } catch (error) {
      console.error(error);
          ...
          >
            ...
            {summary && (
              <motion.div
                ...
                className={`glass border border-primary/20 bg-primary/5 shadow-inner transition-all duration-300 flex flex-col min-h-0 ${isMaximized
                  ? "fixed top-4 right-4 bottom-4 left-[272px] z-20 rounded-2xl shadow-2xl bg-background/95 backdrop-blur-xl"
                  : "rounded-xl overflow-hidden flex-1"
                  }`}
              >
                {/* Tabs & Toolbar */}
