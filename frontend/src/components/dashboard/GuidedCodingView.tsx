...
import {
    ...
    Code2, MessageSquare, Loader, Send, Play, Loader2
} from 'lucide-react';
        ...
        fetch(`/api/tutorial-modules/${moduleId}`)
            ...
            .then(mod => {
                ...
                const dynamicSession = {
                    ...
                    language: mod.content?.starter_code_language || mod.content?.primary_language || 'javascript',
    ...
    const handleSendMessage = async () => {
        ...
        try {
            const response = await fetch("/api/tutorial-modules/ask-video-question", {
                ...
                body: JSON.stringify({
                    videoUrl: moduleUrl,
                    question: `I am currently working on a practice task.
...
User Question: ${msg}`,
            ...
            console.error(error);
            setMessages(prev => [...prev, { role: 'assistant', content: "Sorry, I'm having trouble connecting to the mentor service. Please try again later." }]);
        } finally {
            setIsChatting(false);
        }
    };
                        ...
                        <div className="flex items-center justify-between px-4 pb-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                            <div className="flex items-center gap-2 opacity-70">
                                ...
                                <span className="text-xs font-mono text-white">
                                    {`main.${session.language === 'python' ? 'py' :
                                        session.language === 'javascript' ? 'js' :
                                            session.language === 'typescript' ? 'ts' :
                                                session.language === 'html' ? 'html' :
                                                    session.language === 'css' ? 'css' : 'code'
                                        }`}
                                </span>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4">
                            {messages.map((msg, i) => (
                                <div key={i} className={`fade-in ${msg.role === 'user' ? 'flex justify-end' : ''}`}>
                                    ...
                                    >
                                        <div className="prose prose-invert prose-sm max-w-none prose-p:my-1 prose-pre:my-1 prose-code:bg-background/50 prose-code:px-1">
                            ...
                            {isChatting && (
                                <div className="flex justify-start fade-in">
                                    <div className="max-w-[90%] rounded-2xl p-3.5 text-sm shadow-sm bg-secondary border border-border flex items-center gap-2">
                                        <Loader2 size={14} className="animate-spin text-primary" />
                                        <span className="text-muted-foreground animate-pulse">Mentor is thinking...</span>
                                    </div>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="p-3 border-t border-border bg-secondary/10">
                            <div className="flex gap-2">
                                <input
                                    ...
                                    placeholder={isChatting ? "Mentor is thinking..." : "Need help with this step?"}
                                    disabled={isChatting}
                                    className="flex-1 bg-background border border-border/50 rounded-xl px-3 py-2.5 text-sm outline-none focus:ring-1 focus:ring-primary shadow-inner disabled:opacity-50"
                                />
                                <button
                                    onClick={handleSendMessage}
                                    disabled={isChatting || !userInput.trim()}
                                    className="bg-primary text-primary-foreground p-2.5 rounded-xl hover:bg-primary/90 transition-colors shadow-sm disabled:opacity-50"
                                >
