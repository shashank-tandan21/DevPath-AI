...
import { CheckCircle, LayoutDashboard, Settings, User, Plus, ChevronRight, BookOpen, PenTool, Wrench, Sparkles, Network, LogOut, ChevronLeft, Trash2 } from "lucide-react";
...
const Home = () => {
  ...
  const [tutorialTab, setTutorialTab] = useState<'summary' | 'chat'>('summary');
  ...
  const handleProjectClick = (title: string) => {
    if (activeProject === title) {
      ...
      // If we're in the lessons tab, automatically switch to tutorial-modules view
      if (activeTab === 'lessons') {
        const lesson = lessons.find(l => l.title === title);
        if (lesson) {
          setActiveModuleId(lesson.id);
  ...
  const fetchModules = async () => {
    try {
      const response = await fetch('/api/tutorial-modules');
      const data = await response.json();
      if (Array.isArray(data)) {
        setLessons(data.map((m: { id: string, title: string }) => ({ id: m.id, title: m.title })));
      }
    } catch (err) {
      console.error('Failed to fetch modules for sidebar', err);
    }
  };
  ...
  const handleRenameCommit = (oldName: string, newName: string) => {
    ...
    if (activeTab === 'lessons') {
      if (lessons.some(l => l.title === newName.trim())) return;
      setLessons(lessons.map(l => l.title === oldName ? { ...l, title: newName.trim() } : l));
    } else {
      if (projects.includes(newName.trim())) return;
  ...
  const handleDeleteProject = async (titleOverride?: string) => {
    const titleToDelete = titleOverride || contextMenu?.title;
    if (!titleToDelete) return;

    if (activeTab === 'lessons') {
      const lessonToDelete = lessons.find(l => l.title === titleToDelete);
      if (lessonToDelete) {
        if (window.confirm(`Are you sure you want to delete "${titleToDelete}"?`)) {
          try {
            await fetch(`/api/tutorial-modules/${lessonToDelete.id}`, { method: 'DELETE' });
            setLessons(lessons.filter(l => l.id !== lessonToDelete.id));
            if (activeProject === titleToDelete) setActiveProject(null);
            if (hoveredProject === titleToDelete) setHoveredProject(null);
          } catch (e) {
            console.error(e);
          }
        }
      ...
      if (window.confirm(`Are you sure you want to delete "${titleToDelete}"?`)) {
        setProjects(projects.filter(p => p !== titleToDelete));
        if (activeProject === titleToDelete) setActiveProject(null);
        if (hoveredProject === titleToDelete) setHoveredProject(null);
      }
    }
    ...
    >
            ...
            <div>
              <div className="mb-4 flex flex-col gap-3">
                ...
                <Button
                  ...
                  onClick={() => {
                    if (activeTab === 'lessons') {
                      // For lessons, we typically generate them from YouTube, but if we add manually:
                      const newName = `Lesson ${lessons.length + 1}`;
                      setLessons([...lessons, { id: Date.now().toString(), title: newName }]);
                    } else {
              ...
              <div className="space-y-1">
                {(activeTab === 'lessons' ? lessons.map(l => l.title) : projects).map((title) => (
                  <ProjectItem
                    key={title}
                    title={title}
      ...
      >
        {activeView === "project-flowchart" ? (
          <ProjectFlowchart moduleId={activeModuleId} />
